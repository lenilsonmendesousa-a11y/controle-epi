...
function statusCA(validadeCA){ if(!validadeCA)return{texto:'Aguardando consulta automática',classe:'pendente'}; const h=new Date(); h.setHours(0,0,0,0); const v=new Date(validadeCA+'T00:00:00'); return v<h?{texto:'CA vencido',classe:'vencido'}:{texto:'CA válido',classe:'valido'} }
function abrirConsultaCA(){ window.open('https://caepi.mte.gov.br/internet/consultacainternet.aspx','_blank','noopener,noreferrer') }

async function consultarValidadeCA(ca) {
  const numero = String(ca || '').replace(/\D/g, '')
  if (!numero) throw new Error('Informe o número do CA antes de consultar.')
  const resposta = await fetch(`/api/consulta-ca?ca=${encodeURIComponent(numero)}`)
  if (!resposta.ok) throw new Error('Integração automática com CAEPI ainda não configurada no servidor.')
  const dados = await resposta.json()
  if (!dados.validadeCA) throw new Error('A consulta não retornou validade do CA.')
  return dados.validadeCA
}
...
function App(){
  ...
  function limparMensagens(){ setMensagem(''); setMensagemErro('') }
  async function buscarCAParaCadastro(){
    limparMensagens()
    try{
      const validade = await consultarValidadeCA(epi.ca)
      setEpi({...epi, validadeCA: validade})
      setMensagem('Validade do CA preenchida automaticamente.')
    }catch(err){
      setMensagemErro(err.message + ' Não será preenchida validade falsa. Consulte no CAEPI ou configure a API.')
    }
  }
  async function buscarCAItem(id){
    limparMensagens()
    const item = estoque.find(e=>e.id===id)
    if(!item) return
    try{
      const validade = await consultarValidadeCA(item.ca)
      setEstoque(estoque.map(e=>e.id===id?{...e, validadeCA: validade}:e))
      setMensagem('Validade do CA atualizada automaticamente.')
    }catch(err){
      setMensagemErro(err.message + ' O campo permanecerá pendente até integração real.')
    }
  }

  function cadastrarColaborador(){ limparMensagens(); if(!colaborador.nome||!colaborador.funcao||!colaborador.setor){setMensagemErro('Preencha nome, função e setor.');return} setColaboradores([...colaboradores,{id:gerarId('COL',colaboradores.length),...colaborador,status:'ativo',desligadoEm:'',biometria:null}]); setColaborador({nome:'',funcao:'',setor:''}); setMensagem('Colaborador cadastrado como ativo.') }

  function cadastrarBiometriaReal(id){
    limparMensagens()
    const pessoa=colaboradores.find(c=>c.id===id)
    if(!pessoa)return
    setMensagemErro('Biometria não coletada. Para cadastrar a digital real é necessário instalar/integrar o SDK ou driver do leitor biométrico. O sistema não marcará como coletada sem leitura real.')
  }

  function confirmarBiometriaReal(){
    limparMensagens()
    if(!colabSelecionado){setMensagemErro('Selecione um colaborador ativo para confirmar a biometria.');return}
    if(!colabSelecionado.biometria){setMensagemErro('Este colaborador não possui biometria real cadastrada. Não é possível confirmar.');return}
    setMensagemErro('Confirmação biométrica real ainda não integrada ao leitor. O sistema não marcará como coletada sem leitura real.')
    setBiometriaConfirmada(false)
  }

  function desligarColaborador(id){ const p=colaboradores.find(c=>c.id===id); if(!p)return; if(!window.confirm(`Deseja desligar o colaborador "${p.nome}"?`))return; setColaboradores(colaboradores.map(c=>c.id===id?{...c,status:'desligado',desligadoEm:hojeISO()}:c)); setMensagem('Colaborador desligado.') }
  ...
  return <main>
    <header className="topo"><div className="logo-area"><img src="/weisul-logo.png" alt="Logo Weisul Agro" className="logo"/><div><h1>Sistema de Controle de Entrega de EPI</h1><p>Weisul Agro • Biometria somente com coleta real e validade CA somente por integração/API.</p></div></div><ShieldCheck size={52} color="#5AAA54"/></header>
